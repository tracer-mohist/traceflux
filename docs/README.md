# traceflux Documentation

## For Users

- Installation → user/INSTALLATION.md
- Usage → user/USAGE.md
- Quick Reference → user/QUICK_REF.md
- Output Format → user/OUTPUT-FORMAT.md
- Use Cases → user/USE-CASES.md

## For Contributors

- Getting Started → contributor/GETTING_STARTED.md
- Python Environment → contributor/PYTHON-ENV.md
- Testing Philosophy → contributor/TESTING-PHILOSOPHY.md
- Release Process → contributor/RELEASE.md

## Internal

- Architecture → internal/ARCHITECTURE.md
- Philosophy → internal/PHILOSOPHY.md
- Decisions → internal/DECISIONS.md
- Branch Protection → internal/BRANCH_PROTECTION.md
- GitHub API Reference → internal/GITHUB-BRANCH-PROTECTION-RESEARCH.md

---

**Note**: This documentation structure follows layered architecture (user/contributor/internal) to separate concerns and improve navigation.
